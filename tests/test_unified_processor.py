...
from src.ai.services import ContactExtraction, LeadExtraction

@pytest.fixture
def mock_session():
    session = AsyncMock()
    # SQLAlchemy session.add is synchronous
...
async def test_sync_contact_logic(mock_session):
    ...
    # Case 1: New Contact
    with patch("src.pipeline.unified_processor.find_duplicate", return_value=None):
    ...
    # Case 2: Duplicate Contact
    mock_session.add.reset_mock()
    existing_contact = Contact(id=1, first_name="Old", email="new@example.com")
    with patch("src.pipeline.unified_processor.find_duplicate", return_value=existing_contact), \
         patch("src.pipeline.unified_processor.merge_contact_fields") as mock_merge:
...
async def test_sync_lead_logic(mock_session):
    ...
    with patch("src.pipeline.unified_processor.find_duplicate", return_value=None):
        mock_session.execute.return_value = MagicMock(scalar_one_or_none=lambda: None) # No link exists
...
async def test_maintenance_reindex_dirty(mock_session):
    ...
    # Mock return values for multiple execute calls
    ...
    # First call is for dirty contacts, subsequent for messages
...
async def test_maintenance_index_messages(mock_session):
    ...
    with patch("src.pipeline.unified_processor.generate_embedding", return_value=[0.2]*1024):
...
async def test_update_all_lead_scores(mock_session):
    ...
    # Mock SettingsService return values
    with patch("src.pipeline.unified_processor.SettingsService") as mock_settings_cls, \
        ...
        # Mocking the async context manager behavior of get_session
        ...
        # No longer asserting commit here because mock_get_session doesn't execute the real commit logic
